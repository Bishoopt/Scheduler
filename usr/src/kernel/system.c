...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->group=rpp->group;
  rpc->starving_time=rpp->starving_time;
  return(OK);
}
...
message *m_ptr;
{
	...
	for(I = NR_TASKS; I < NR_TASKS + NR_PROCS; I++)
		if(proc[I].p_pid == m_ptr->m1_i1 && proc[I].p_priority == PPRI_USER)
			{
				rp=&proc[I];
				break;
			}
...
/*
	int do_setgroup(m_ptr);
	Zwraca
	1 gdy prawidlowo zmieniona
	EINVAL gdy proces nie został odnaleziony lub  podano zla grupe
*/
PRIVATE int do_setgroup(m_ptr)
message * m_ptr;
{
	struct proc * rp= NIL_PROC;
	struct proc * xp= rdy_head[USER_Q];
	int new_group = m_ptr->m2_l2;
	int I;
	if(new_group!=GROUP_NORMAL && new_group!=GROUP_PRIVI && new_group!=GROUP_BACKG)
		return EINVAL;
	for(I = NR_TASKS; I < NR_TASKS + NR_PROCS; I++)
		if(proc[I].p_pid == m_ptr->m1_i1 && proc[I].p_priority == PPRI_USER)
			{

 ...
 */
  ...
  static struct proc* previous=NIL_PROC;
	struct proc *normal=NIL_PROC,*privi=NIL_PROC, *backg=NIL_PROC;
	struct proc *iter=rdy_head[USER_Q];
	struct proc *previousNormal,*previousPrivi,*previousBackg,*previousI;
	struct proc *setPROC=NIL_PROC;
	if (rdy_head[USER_Q] == NIL_PROC) return;


	if(previous!=NIL_PROC)
	{
	...
	{
			/* zrzucamy aktualny na koniec */
	  	rdy_tail[USER_Q]->p_nextready = rdy_head[USER_Q];
  		rdy_tail[USER_Q] = rdy_head[USER_Q];
  		rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
  		rdy_tail[USER_Q]->p_nextready = NIL_PROC;
  		/* */
  	previousI=rdy_head[USER_Q];
  	iter=rdy_head[USER_Q];
		while(iter!=NIL_PROC)
		{
			if(normal==NIL_PROC && iter->group==GROUP_NORMAL)
			{
			  previousNormal=previousI;
				normal=iter;
      }
			else if(privi==NIL_PROC && iter->group==GROUP_PRIVI)
			{
			  previousPrivi=previousI;
				privi=iter;
      }
			else if(backg==NIL_PROC && iter->group==GROUP_BACKG)
			{
			  previousBackg=previousI;
				backg=iter;
      }
      --iter->starving_time;
      previousI=iter;
		...
		if(privi!=NIL_PROC && backg!=NIL_PROC && normal==NIL_PROC) /*regula 6*/
		{
		  /*uruchom tylko privi*/
				if(privi!=rdy_head[USER_Q])/*nie mamy go na glowe*/
				{
					/*jestesmy na koncu*/
					if(rdy_tail[USER_Q]==privi)
					{
						previousPrivi->p_nextready=NIL_PROC;
						rdy_tail[USER_Q]=previousPrivi;
					}else
					{
						previousPrivi->p_nextready=privi->p_nextready;
					}/*usuwamy go z kolejki*/
					privi->p_nextready=rdy_head[USER_Q];
					rdy_head[USER_Q]=privi;
				}
		       pick_proc();
		       return;
		}else if(normal!=NIL_PROC && privi==NIL_PROC && backg!=NIL_PROC) /*regula 5*/
		{
		/* nie dopuscic do zaglodzenia*/
		
		  if(backg->starving_time<=0)/* zaglodzenie uruchom backg */
		  {
		    backg->starving_time=DEFAULT_STARVING_TIME;
		    if(backg!=rdy_head[USER_Q])/*nie mamy go na glowe*/
				{
					/*jestesmy na koncu*/
					if(rdy_tail[USER_Q]==backg)
					{
						previousBackg->p_nextready=NIL_PROC;
						rdy_tail[USER_Q]=previousBackg;
					}else
					{
						previousBackg->p_nextready=backg->p_nextready;
					}/*usuwamy go z kolejki*/
					backg->p_nextready=rdy_head[USER_Q];
					rdy_head[USER_Q]=backg;
				}
		  }else /* uruchamiaj normale */
		  {
		  	
		  	if(normal!=rdy_head[USER_Q])/*nie mamy go na glowe*/
				{
					/*jestesmy na koncu*/
					if(rdy_tail[USER_Q]==normal)
					{
						previousNormal->p_nextready=NIL_PROC;
						rdy_tail[USER_Q]=previousNormal;
					}else
					{
						previousNormal->p_nextready=normal->p_nextready;
					}/*usuwamy go z kolejki*/
					normal->p_nextready=rdy_head[USER_Q];
					rdy_head[USER_Q]=normal;
				}
			}
		}
		else if(normal!=NIL_PROC && privi!=NIL_PROC)/*regula 4 i 7*/
		{
				...
				/* wiemy ze setPPROC wskazuje na ten na co ma byc na poczatku i previousI na porzednika setPROCA */
				if(setPROC!=rdy_head[USER_Q])/*nie mamy go na glowe*/
				{
					/*jestesmy na koncu*/
					if(rdy_tail[USER_Q]==setPROC)
					{
						previousI->p_nextready=NIL_PROC;
						rdy_tail[USER_Q]=previousI;
					}else
					{
						previousI->p_nextready=setPROC->p_nextready;
					}/*usuwamy go z kolejki*/
					setPROC->p_nextready=rdy_head[USER_Q];
					rdy_head[USER_Q]=setPROC;
				}
				
		    if(rdy_head[USER_Q]==privi)
		    {
		        pick_proc();
		        previous=privi;
		        return;
